"""
Emergency schema fix script for quotations table.
...
def check_and_fix_schema():
    """Check quotations table schema and add missing columns."""

    with engine.connect() as connection:
        inspector = inspect(engine)

        # Check if quotation table exists
        tables = inspector.get_table_names()
        if 'quotation' not in tables:
            print("[X] quotation table does not exist")
            return False

        print("[OK] quotation table exists")

        # Get existing columns
        columns = inspector.get_columns('quotation')
        existing_columns = {col['name'] for col in columns}

        print(f"\nExisting columns: {sorted(existing_columns)}")

        # Required columns based on Quotation model
        required_columns = {
        ...
        for col in sorted(missing_columns):
            try:
                if col == 'discount':
                    sql = text("ALTER TABLE quotation ADD COLUMN discount NUMERIC(10, 2) DEFAULT 0.00")
                elif col == 'balance':
                    sql = text("ALTER TABLE quotation ADD COLUMN balance NUMERIC(10, 2)")
                elif col == 'advance_payment':
                    sql = text("ALTER TABLE quotation ADD COLUMN advance_payment NUMERIC(10, 2) DEFAULT 0.00")
                else:
...
if __name__ == "__main__":
    print("="*60)
    print("Quotations Table Schema Fix")
    print("="*60 + "\n")
